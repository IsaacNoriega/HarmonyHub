...
const deleteProjectButton = document.getElementsByClassName('deleteProject');

for (const modalButton of deleteProjectButton) {
  modalButton.addEventListener('click', (event) => {
    event.stopPropagation(); // Detener la propagación del evento hacia el modal

    const modalProject = document.getElementById('modalDeleteProject');
    modalVisible = !modalVisible; // Alternar el estado de visibilidad del modal
    modalProject.style.display = modalVisible ? 'block' : 'none'; // Mostrar u ocultar el modal según su estado de visibilidad
...
document.getElementById('deleteProjectForm').addEventListener('submit', function (event) {
  event.preventDefault();
  const projectName = document.getElementById('projectNameToDelete').value;

const cookiesArray = document.cookie.split('; '); // Dividir las cookies en un array
const cookies = {}; // Objeto para almacenar las cookies
// Iterar sobre cada cookie en el array
for (const cookie of cookiesArray) {
    const [name, value] = cookie.split('='); // Dividir la cookie en nombre y valor

    // Verificar si el nombre de la cookie es 'email'
    if (name === 'email') {
        cookies[name] = decodeURIComponent(value); // Almacenar el valor de la cookie 'email'

        break; // No es necesario seguir buscando más cookies
    }
}

const email = cookies.email; // Obtener el valor de la cookie 'email'
  ...
  $.ajax({
    ...
    contentType: 'application/json', // Importante: no configurar contentType cuando se usa FormData
    success: function (response) {
      console.log('Success')
